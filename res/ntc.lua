...
function ntc.name(color)
    ...
    for k,v in pairs(names) do
        ...
        h1,s1,l1 = Color.fromHex(k):toHSL()
        ndf1 = pow(r-r1, 2) + pow(g-g1, 2) + pow(b-b1, 2)
        ndf2 = pow(h-h1, 2) + pow(s-s1, 2) + pow(l-l1, 2)
        ndf = ndf1 + ndf2 * 2
        if df<0 or df>ndf then
            df = ndf
            cl = k
        end
    end

    return cl or "000000", names[cl] or "Invalid color:"..color:toHex()
end
